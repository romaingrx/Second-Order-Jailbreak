...
class TransformersLlamaConversational(IntelligenceBackend):
    ...
    def __init__(self, model: str, device: int = 'cuda', merge_other_agents_as_one_user: bool = True, **kwargs):
        super().__init__(model=model, device=device, merge_other_agents_as_one_user=merge_other_agents_as_one_user, **kwargs)
        self.model = model
        self.device = device
        ...
        tokenizer = AutoTokenizer.from_pretrained(self.model, device_map=self.device, use_fast=True)
